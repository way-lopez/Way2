const SAVE_KEY = "wayly_starter14_balance_v1";
  ...
  const renderSide = (wrap, items) => {
    ...
    for(let i=0;i<3;i++){
      ...
      if(!item){
        ...
        const ctype = classifyCardType(card);
        div.className = "played-slot " + ctype;
        div.innerHTML = `<div class="slot-icon">${iconForCard(card)}</div><div class="slot-name">${card ? card.name : item.id}</div><div class="slot-meta">custo ${card ? card.cost : "?"}</div>`;
      }
...
function playPlayerCard(handIndex){
  ...
  gameState.roundPlayed.player.push({ id: cardId });
  if(gameState.roundPlayed.player.length > 3) gameState.roundPlayed.player = gameState.roundPlayed.player.slice(-3);
  renderPlayedHistory();
...
function runEnemyTurn(){
  ...
  const performEnemyCard = () => {
    ...
    b.enemy.hand.forEach((id, idx) => {
      const c = getCard(id);
      if(!c) return;
      const s = scoreCardForBot(c, b.enemy, b.player);
    ...
    gameState.roundPlayed.enemy.push({ id: cardId });
    if(gameState.roundPlayed.enemy.length > 3) gameState.roundPlayed.enemy = gameState.roundPlayed.enemy.slice(-3);
    renderPlayedHistory();
    ...
    // If enemy still has energy and a viable card, play one more after delay
    let hasAnother = false;
    b.enemy.hand.forEach((id) => {
      const c = getCard(id);
      if(c && c.cost <= b.enemy.energy && scoreCardForBot(c, b.enemy, b.player) >= 0) hasAnother = true;
    });
...
function buildCardInner(card, detailed){
  ...
  return `
    <div class="card-preview">
      ...
      <div class="card-mini-foot"><span>${card.rarity}</span><span>${detailed ? "Detalhes" : ""}</span></div>
    </div>
  `;
}
