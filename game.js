...
function renderPlayedHistory(){
  ...
  const renderSide = (wrap, items) => {
    ...
    for(let i = 0; i < 3; i++){
      ...
      if(!item){
        ...
        div.innerHTML = `<div class="slot-icon">${icon}</div><div class="slot-name">${title}</div><div class="slot-meta">custo ${cost}</div>`;
      }
  ...
  try{
    ...
    enemyWrap.innerHTML = "";
    for(const wrap of [playerWrap, enemyWrap]){
      for(let i = 0; i < 3; i++){
        const div = document.createElement("div");
        div.className = "played-slot empty";
        div.innerHTML = '<div class="slot-icon">✦</div><div class="slot-name">—</div><div class="slot-meta">vazio</div>';
    ...
    showDebug("Erro ao renderizar slots: " + (err && err.message ? err.message : err));
  }
}
