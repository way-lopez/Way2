...
function loadSave(){
  try{
    ...
    const merged = { ...defaultSave(), ...parsed, options: { ...defaultSave().options, ...(parsed.options || {}) } };
    merged.playerChoices = Array.isArray(merged.playerChoices) ? merged.playerChoices.filter(Boolean) : [];
    merged.botChoices = Array.isArray(merged.botChoices) ? merged.botChoices.filter(Boolean) : [];
    merged.removedStarterIds = Array.isArray(merged.removedStarterIds) ? [...new Set(merged.removedStarterIds.filter(Boolean))] : [];
    return merged;
  }catch(e){
    return defaultSave();
  }
...
function getPlayerDeckList(){
  return buildDeckFromChoices(gameState.save.playerChoices, gameState.save);
}

function getBotDeckList(){
  return buildDeckFromChoices(gameState.save.botChoices, gameState.save);
}

function getActiveStarterIdsInDeck(choiceIds = gameState.save.playerChoices){
  const removed = new Set(gameState.save.removedStarterIds || []);
  return [...new Set(buildDeckFromChoices(choiceIds, gameState.save).filter(id => getStarterCardIds().includes(id) && !removed.has(id)))];
}
...
function renderMenuStats(){
  ...
  el("statDeckSize").textContent = getPlayerDeckList().length;
...
function openDeck(){
  ...
  grid.innerHTML = "";
  const unlocked = new Set([...getStarterCardIds(), ...(gameState.save.playerChoices || [])]);
  el("deckCount").textContent = getPlayerDeckList().length;
...
function startNewGame(){
  const ok = confirm("Iniciar novo jogo? Isso apaga a jornada atual.");
  if(!ok) return;
...
function returnToMenu(){
  if(gameState.battle){
    gameState.save.lastNotice = "Batalha pausada. Você pode continuar de onde parou ou iniciar um novo jogo.";
    persistPausedBattle();
  }
  goMenu();
}
...
function startEvent(){
  const candidates = [];
  const starterInDeck = getActiveStarterIdsInDeck();
...
function renderEvent(){
  ...
  if(type === "removeWeak"){
    ...
    const playerDeck = getActiveStarterIdsInDeck();
    if(playerDeck.length === 0){
      return startFallbackEventReward("Nenhuma carta inicial disponível para remoção. Você seguirá direto para a próxima batalha.");
    }
    ...
    if(unique.length === 0){
      return startFallbackEventReward("Você ainda não tem cartas escolhidas suficientes para duplicar. A jornada segue.");
    }
    unique.forEach(id => {
      const card = getCard(id);
      const vis = getCardVisual(card);
    ...
    const playerDeck = getActiveStarterIdsInDeck();
    if(playerDeck.length === 0){
      return startFallbackEventReward("Nenhuma carta inicial disponível para transmutação. A jornada segue.");
    }
    playerDeck.forEach(id => {
      const card = getCard(id);
      const div = document.createElement("div");
...
function startFallbackEventReward(message){
...
function applyEvent(type, payload){
  if(type === "removeWeak"){
    if(!gameState.save.removedStarterIds.includes(payload)){
      gameState.save.removedStarterIds.push(payload);
    }
  } else if(type === "duplicate"){
...
function finishBattle(win){
  ...
  if(win){
    ...
    el("resultStats").innerHTML = `
      ...
      <div class="stat-row"><span>Cartas ativas no deck</span><strong>${getPlayerDeckList().length}</strong></div>
