...
function iconForCard(card){
  const type = card ? classifyCardType(card) : "special";
  if(type === "attack") return "⚔️";
  if(type === "defense") return "🛡️";
  if(type === "buff") return "✨";
  if(type === "debuff") return "☠️";
...
function renderPlayedHistory(){
  ...
  const renderSide = (wrap, items) => {
    ...
    for(let i = 0; i < 3; i++){
      const raw = shown[i];
      const item = normalizePlayedItem(raw);
      const div = document.createElement("div");
      if(!item){
        div.className = "played-slot empty";
        div.innerHTML = '<div class="slot-icon">✦</div><div class="slot-name">—</div><div class="slot-meta">vazio</div>';
        ...
        const card = getCard(item.id);
        const ctype = card ? classifyCardType(card) : "special";
        const title = card ? card.name : (item.id || "Carta");
...
function runEnemyTurn(){
  ...
  try{
    ...
    const chooseEnemyCardIndex = () => {
      ...
      if(bestIndex < 0 || bestScore < 0) return -1;
      return bestIndex;
    };
    ...
    const performEnemyCard = () => {
      ...
      const hasAnother = b.enemy.hand.some((id) => {
        const c = getCard(id);
