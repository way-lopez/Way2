...
function getRemovedStarterSet(saveState){
  return new Set((saveState?.removedStarterIds || []).filter(Boolean));
}

function buildStarterDeck(saveState = null){
  const removed = getRemovedStarterSet(saveState);
  const deck = [];
...
function buildDeckFromChoices(choiceIds, saveState = null){
  const deck = buildStarterDeck(saveState);
